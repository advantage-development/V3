...
WalletModel::SendCreditsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    CAmount total = 0;
    ...
    foreach (const SendCreditsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered advantagecoin address / amount:
...
WalletModel::SendCreditsReturn WalletModel::sendCredits(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCreditsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal advantagecoin:URI (advantagecoin:XyZ...?message=example)
            {
...
void WalletModel::listCredits(std::map<QString, std::vector<COutput> >& mapCredits) const
{
    ...
    BOOST_FOREACH (const COutput& out, vCredits) {
        ...
        if (!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCredits[QString::fromStdString(CBitcoinAddress(address).ToString())].push_back(out);
    }
}
