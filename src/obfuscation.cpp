...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeAdvantageCoinAmount * COIN - pwalletMain->GetAnonymizedBalance();
    ...
    if (!pwalletMain->SelectCreditsDark(nValueMin, nBalanceNeedsAnonymized, vCredits, nValueIn, 0, nObfuscationRounds)) {
        ...
        if (pwalletMain->SelectCreditsDark(nValueMin, 9999999 * COIN, vCredits, nValueIn, -2, 0)) {
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("advantagecoin-obfuscation");
