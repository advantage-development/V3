...
map<CBitcoinAddress, vector<COutput> > CWallet::AvailableCreditsByAddress(bool fConfirmed, CAmount maxCoinValue)
{
    vector<COutput> vCredits;
    AvailableCredits(vCredits, fConfirmed);

    map<CBitcoinAddress, vector<COutput> > mapCredits;
    BOOST_FOREACH (COutput out, vCredits) {
        ...
        mapCredits[CBitcoinAddress(address)].push_back(out);
    }
        ...
        BOOST_FOREACH (CAmount v, obfuScationDenominations) {
            BOOST_FOREACH (const COutput& out, vCredits) {
                if (out.tx->vout[out.i].nValue == v                                               //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1 * COIN) + 100 //round the amount up to .1 USDA over
                    ) {
                    CTxIn vin = CTxIn(out.tx->GetHash(), out.i);
                    int rounds = GetInputObfuscationRounds(vin);
                    // make sure it's actually anonymized
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-advantagecoin-address
                    CScript scriptChange;
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew, unsigned int& nTxNewTime)
{
	...
	if (pIndex0->nHeight > 1) {
		// Finally pay the 
		CBitcoinAddress address(Params().GetTreasuryRewardAddress());
...
string CWallet::PrepareObfuscationDenominate(int minRounds, int maxRounds)
{
    ...
    */
    if (minRounds >= 0) {
        if (!SelectCreditsByDenominations(obfuScationPool.sessionDenom, 0.1 * COIN, OBFUSCATION_POOL_MAX, vCredits, vCredits2, nValueIn, minRounds, maxRounds))
...
void CWallet::AutoCombineDust()
{
    ...
    map<CBitcoinAddress, vector<COutput> > mapCreditsByAddress = AvailableCreditsByAddress(true, 0);

    //credits are sectioned by address. This combination code only wants to combine inputs that belong to the same address
    for (map<CBitcoinAddress, vector<COutput> >::iterator it = mapCreditsByAddress.begin(); it != mapCreditsByAddress.end(); it++) {
